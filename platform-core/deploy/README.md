...
5. Verificar la configuración e instalar/activar el proceso:
...
```bash
curl -fsS https://cuentas.stylocamion.com/health
curl -fsS https://cuentas.stylocamion.com/health/ready
pm2 status
pm2 logs stylocamion-platform --lines 100
```

El primer endpoint confirma que el proceso está vivo. El segundo sólo devuelve
`200` cuando el backend puede consultar la base de datos. Si faltan secretos, el
proceso permanece en modo de configuración y muestra una página segura en vez
de provocar un `502`.

8. Probar acceso por teléfono, creación de cuenta, orden, pago, webhook,
recuperación y validación de QR.
