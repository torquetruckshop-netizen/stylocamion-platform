...
test('detecta caída crítica de pagos contra el mismo tramo de ayer',()=>{
 const facts=[];
 for(let i=0;i<10;i++) facts.push(payment(`prev-${i}`,`2026-08-14T${String(i+1).padStart(2,'0')}:00:00Z`));
 facts.push(payment('today-1','2026-08-15T10:00:00Z'));
...
test('sin variaciones fuertes queda normal',()=>{
 const facts=[payment('a','2026-08-14T10:00:00Z'),payment('b','2026-08-15T10:00:00Z')];
 const r=detectMetricAnomalies({facts,period:'DAY',now:new Date('2026-08-15T12:00:00Z')});
