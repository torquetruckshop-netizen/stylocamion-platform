...
test('sesion recordada queda activa por largo plazo',()=>{
  ...
  const {token,session}=createPersistentSession(USER_ID,{remember:true,device_label:'iPhone'},now);
...
test('actividad renueva silenciosamente la expiracion',()=>{
  const now=new Date('2026-08-15T12:00:00Z');
  const {session}=createPersistentSession(USER_ID,{remember:true},now);
...
test('logout revoca la sesion',()=>{
  const {session}=createPersistentSession(USER_ID,{remember:true});
